...
def main(input1,input2,input3,input4):
    ...
    while True: 
        ...
        if(float(mark_price) > BREAK_PRICE and float(mark_price) < (BREAK_PRICE + 1) or (float(mark_price) > (BREAK_PRICE + 3))):
            ...
            print(yu)
            orderId = yu["orderId"]
            mo = "BUY INITIATED OF " + str(tha_dough) + " IN CURRENCY  " + currency + " AT " + str(mark_price) + " ORDER ID: " +  str(orderId)
            lg.logg(mo)
            counter1 = 0
            still1 = SECONDS

            
            message = f"BUY INITIATED OF {tha_dough} IN CURRENCY {currency} AT {mark_price}  BTC LONG BOT"
            url = f"https://api.telegram.org/bot{TELE_TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
            url2 = f"https://api.telegram.org/bot{TELE_TOKEN2}/sendMessage?chat_id={chat_id2}&text={message}"
            requests.get(url).json() 
            requests.get(url2).json()
            ...
            while True:
                ...
                if(float(mark_price) < (BREAK_PRICE - diff)):
                    print("AAAAAAAAAHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH")
                    yu = s.sell(client,tha_dough, currency)
                    orderId = yu["orderId"]
                    nogh = "SELL INITIATED OF " + str(tha_dough) + " IN CURRENCY" + currency + " AT " + str(mark_price) + " ORDER ID: " + str(orderId)
                    lg.logg(mo)
                    break
            message = f"SELL INITIATED OF {tha_dough} IN CURRENCY {currency} AT {mark_price} BTC LONG BOT"
            url = f"https://api.telegram.org/bot{TELE_TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
            url2 = f"https://api.telegram.org/bot{TELE_TOKEN2}/sendMessage?chat_id={chat_id2}&text={message}"
            requests.get(url).json()
            requests.get(url2).json()
