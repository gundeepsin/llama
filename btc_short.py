...
#sd
    ...
    while True: 
        ...
        if(float(mark_price) < BREAK_PRICE and float(mark_price) > (BREAK_PRICE - 1) or (float(mark_price) < (BREAK_PRICE - 3))):
            ...
            print(yu)
            orderId = yu["orderId"]
            mo = "SHORT INITIATED OF " + str(tha_dough) + " IN CURRENCY  " + currency + " AT " + str(mark_price) + " BTC SHORT BOT"
            lg.logg(mo)
            counter1 = 0
            still1 = SECONDS

            
            message = f"SHORT INITIATED OF {tha_dough} IN CURRENCY {currency} AT {mark_price} BTC SHORT BOT"
            url = f"https://api.telegram.org/bot{TELE_TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
            url2 = f"https://api.telegram.org/bot{TELE_TOKEN2}/sendMessage?chat_id={chat_id2}&text={message}"
            requests.get(url).json() 
            requests.get(url2).json()
            ...
            while True:
                ...
                if(float(mark_price) > (BREAK_PRICE + diff)):
                    ...
                    orderId = yu["orderId"]
                    nogh = "SHORT DROPPED OF " + str(tha_dough) + " IN CURRENCY" + currency + " AT " + str(mark_price) + " BTC SHORT BOT"
                    lg.logg(mo)
                    break
            message = f"SHORT DROPPED OF {tha_dough} IN CURRENCY {currency} AT {mark_price} BTC SHORT BOT"
            url = f"https://api.telegram.org/bot{TELE_TOKEN}/sendMessage?chat_id={chat_id}&text={message}"
            url2 = f"https://api.telegram.org/bot{TELE_TOKEN2}/sendMessage?chat_id={chat_id2}&text={message}"
            requests.get(url).json()
            requests.get(url2).json()
